  ...
  <div class="app-container">
    <el-row style="display: flex; height: 100vh; overflow: hidden; ">
      <el-col style="flex: 0 0 36%; height: 100%; padding-right: 0px; overflow-y: auto;">
        <el-row :gutter="0" style="margin-bottom: 0px; height: 38vh; overflow: hidden;">
          ...
          <el-row :gutter="10" class="mb8" :style="{height: '2vh',  marginTop: '0px'}">
            <el-col :span="1.5">
              <el-button type="primary" plain icon="el-icon-plus" size="mini" :disabled="!multiple" @click="handleAdd">新增</el-button>
            </el-col>
            <el-col :span="1.5">
            ...
            <el-col :span="1.5">
              <el-button type="warning" plain icon="el-icon-accept" size="mini" :disabled="multiple" @click="handleAccept">采纳</el-button>
            </el-col>
            <el-col :span="1.5">
      ...
      <el-col style="flex: 1; height: 100%; overflow-y: auto;">
        ...
        <el-row :gutter="10" class="mb8" :style="{height: '2vh',  marginTop: '0px'}">
          <el-col :span="1.5">
            <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="admin_single" @click="handleUpdateAdmin">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="admin_multiple" @click="handleDelete">删除</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table height="82.5vh" stripe border v-loading="admin_loading" :data="searchQaList" @selection-change="handleAdminSelectionChange"
          ...
          <el-table-column label="问题状态"  width="80" align="center" prop="status">
            <template slot-scope="scope">
                <span :class="`status-${scope.row.status}`">
                  {{ scope.row.status }}
                </span>
          ...
          <el-table-column label="操作" align="center" width="120" fixed="right" >
            <template slot-scope="scope">
              ...
              <el-col :span="1.5">
                <el-button @click="handleUpdateAdmin(scope.row)" type="text" size="small">编辑</el-button>
              </el-col>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 问题创建和修改表单 -->
    <el-dialog :title="question_title" :visible.sync="question_open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="question_form" :model="question_form" :rules="questionRules" label-width="80px">
        <el-row :gutter="20">
          <el-col :span="10">
            <el-form-item label="用户姓名" prop="createBy">
              <el-input v-model="question_form.createBy" :disabled="true"/>
            </el-form-item>
          </el-col>
          <el-col :span="14">
            <el-form-item label="用户工号" prop="createId">
              <el-input v-model="question_form.createId" :disabled="true"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="10">
            <el-form-item label="房间号" prop="roomNumber">
              <el-input v-model="question_form.roomNumber" placeholder="请输入房间号"/>
            </el-form-item>
          </el-col>
          <el-col :span="14">
            <el-form-item label="用户电话" prop="telephone">
              <el-input v-model="question_form.telephone" placeholder="请输入电话号码"/>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="10">
          <el-col :span="10">
            <el-form-item label="密级" prop="classification">
              <el-select v-model="question_form.classification" placeholder="请选择密级" value-key="securityId" @change="handleQuestionSecurityChange" filterable>
          ...
          <el-col :span="14">
            <el-form-item label="问题类别" prop="type">
              <el-select v-model="question_form.type" placeholder="请选择问题类别" value-key="typeId" @change="handleQuestionTypeChange" filterable>
        ...
        <el-form-item label="问题标题" prop="title">
          <el-input v-model="question_form.title" placeholder="请输入问题标题"/>
        </el-form-item>

        <el-form-item label="严重程度" prop="severity">
          <el-radio-group v-model="question_form.severity">
        ...
        <el-form-item label="问题内容" prop="questionContent">
          <el-input v-model="question_form.questionContent" type="textarea" :rows="4" placeholder="请输入问题内容" />
        </el-form-item>
        <el-form-item label="问题附件" prop="questionAttachments">
            <el-upload
              ref="questionUpload"
              :file-list="question_form.questionAttachList"
              ...
              :on-success="handleUploadQuestionSuccess"
              :limit="3"
    ...
    <!-- 管理员修改表单 -->
    <el-dialog :title="admin_title" :visible.sync="admin_open" width="700px" v-dialogDrag append-to-body>
      <el-form ref="admin_form" :model="admin_form" :rules="adminRules" label-width="80px">
        <el-row :gutter="10">
          <el-col :span="8">
            <!-- 用户姓名  -->
            <el-form-item label="用户姓名" prop="createBy">
              <el-input v-model="admin_form.createBy" placeholder="请输入用户姓名"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 用户工号  -->
            <el-form-item label="用户工号" prop="createId">
              <el-input v-model="admin_form.createId" placeholder="请输入用户工号"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 部门  -->
            <el-form-item label="部门" prop="department">
              <el-input v-model="admin_form.department" placeholder="请输入用户部门"/>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="10">
          <el-col :span="8">
            <!-- 房间号  -->
            <el-form-item label="房间号" prop="roomNumber">
              <el-input v-model="admin_form.roomNumber" placeholder="请输入房间号"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 用户电话  -->
            <el-form-item label="用户电话" prop="telephone">
              <el-input v-model="admin_form.telephone" placeholder="请输入电话号码"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 问题状态  -->
            <el-form-item label="问题状态" prop="status">
              <el-select v-model="admin_form.status" placeholder="请选择问题状态">
        ...
        <el-row :gutter="10">
          <el-col :span="6">
            <!-- 密级  -->
            <el-form-item label="密级" prop="classification">
              <el-select v-model="admin_form.classification" placeholder="请选择密级" value-key="securityId" @change="handleAdminSecurityChange" filterable>
                <el-option v-for="item in securityOptions" :key="item.securityId" :label="item.securityName" :value="item"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <!-- 问题类别  -->
            <el-form-item label="问题类别" prop="type">
              <el-select v-model="admin_form.type" placeholder="请选择问题类别" value-key="typeId" @change="handleAdminTypeChange" filterable>
                <el-option v-for="item in typeOptions" :key="item.typeId" :label="item.type" :value="item"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 严重程度  -->
            <el-form-item label="严重程度" prop="severity">
              <el-select v-model="admin_form.severity" placeholder="请选择严重程度">
        ...
        <el-row :gutter="10">
          <el-col :span="8">
            <!-- 回答者姓名  -->
            <el-form-item label="答者姓名" prop="updateBy">
              <el-input v-model="admin_form.updateBy" placeholder="请输入回答用户姓名"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 回答者工号  -->
            <el-form-item label="答者工号" prop="updateId">
              <el-input v-model="admin_form.updateId" placeholder="请输入用户工号"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <!-- 是否可解决  -->
            <el-form-item label="可解决否" prop="canSolve">
              <el-select v-model="admin_form.canSolve" label-width="100px" placeholder="请选择">
                <el-option label="可解决" value="可解决"></el-option>
        ...
        <!-- 问题标题  -->
        <el-form-item label="问题标题" prop="title">
          <el-input v-model="admin_form.title" placeholder="请输入问题标题"/>
        </el-form-item>
        <!-- 问题内容  -->
        <el-form-item label="问题内容" prop="questionContent">
          <el-input v-model="admin_form.questionContent" type="textarea" :rows="4" placeholder="请输入问题内容" />
        </el-form-item>

        <!-- 回答内容  -->
        <el-form-item label="回答内容" prop="answerContent">
          <el-input v-model="admin_form.answerContent" type="textarea" :rows="4" placeholder="请输入回答内容" />
        </el-form-item>

        <!-- 问题附件上传  -->
        <el-form-item label="问题附件" prop="questionAttachments">
          <el-upload
            ref="adminQuestionUpload"
            :file-list="admin_form.questionAttachList"
            :action= "uploadFileUrl"
            :multiple="true"
            :on-preview="handleFormPreview"
            :headers="headers"
            :before-upload="handlebeforeUpload"
            :on-remove="handleAdminQuestionRemove"
            :on-error="handleUploadError"
            :on-success="handleAdminUploadQuestionSuccess"
            :limit="3"
            :on-exceed="handleExceed"
            list-type="picture">
            <el-button size="small" type="primary">点击上传</el-button>
      ...
      <el-form-item label="回答附件" prop="answerAttachments">
          <el-upload
            ref="adminAnswerUpload"
            :file-list="admin_form.answerAttachList"
            :action= "uploadFileUrl"
            :multiple="true"
            :on-preview="handleFormPreview"
            :headers="headers"
            :before-upload="handlebeforeUpload"
            :on-error="handleUploadError"
            :on-remove="handleAdminAnswerRemove"
            :on-success="handleAdminUploadAnswerSuccess"
            :limit="3"
            :on-exceed="handleExceed"
            list-type="picture">
            <el-button size="small" type="primary">点击上传</el-button>
    ...
    <!-- 导出选择表单 -->
    <el-dialog title="导出数据" :visible.sync="export_open" width="400px" v-dialogDrag append-to-body>
      <el-form ref="export_form" :model="export_form" :rules="exportRules" label-width="70px">
        <!-- 时间范围 -->
        <el-form-item label="时间范围" prop="dataRange" label-width="100px">
          <el-select v-model="export_form.dataRange" placeholder="请选择导出范围">
            <el-option label="最近一周" value="最近一周"/>
            <el-option label="最近一月" value="最近一月"/>
            <el-option label="最近一年" value="最近一年"/>
            <el-option label="全部" value="全部"/>
          </el-select>
        </el-form-item>
        <!-- 保存模式 -->
        <el-form-item label="保存模式" prop="saveMode" label-width="100px">
          <el-select v-model="export_form.saveMode" placeholder="请选择保存模式">
            <el-option label="精简模式" value="精简模式"/>
            <el-option label="标准模式" value="标准模式"/>
            <el-option label="全量模式" value="全量模式"/>
          </el-select>
        </el-form-item>
        <!-- 保存目录 -->
        <el-form-item label="保存目录" prop="savePath" label-width="100px">
          <el-input v-model="export_form.savePath" placeholder="请输入保存目录文件夹" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import {refreshCache, openNotify, closeNotify, setNotify, markNotified, listSecurityOption, listTypeOption, getQa, updateVisits, getUser, updateRemoveRes, convertToPdf, getRepliesByRecordId, addReply, thumbsUpReply, delReply, getReplyFloor, getReplyPage} from "@/api/qaSystem/qaCommon";
...
export default {
  ...
  methods: {
    ...
    // 下拉密级选择变化
    handleQuestionSecurityChange(val){
      this.question_form.classification = val.securityName;
    },
    handleAnswerSecurityChange(val){
      this.answer_form.classification = val.securityName;
    },
    handleAdminSecurityChange(val){
      this.admin_form.classification = val.securityName;
    },
    /** 左侧查询参数列表：提问者、回答者、管理者后端方法需要区别 */
    getList() {
      this.loading = true;
      listAdminQa(this.queryParams).then(response => {
    ...
    openMessage(){
      openNotify().then(response => {
        ...
        console.error("打开消息提醒功能失败", error);
      })
    },
    ...
    closeMessage(){
      closeNotify().then(response => {
        ...
        console.error("关闭消息提醒功能失败", error);
      })
    },

    /** 查询需提醒的问题回答列表 */
    getNotifyList(){
      listAdminNotify().then(response => {
    ...
    getSearchList() {
      ...
      this.searchQueryParams.pageType = "admin";
      listQa(this.searchQueryParams).then(response => {
    ...
    handleAdd(_this, item, data) {
      ...
      this.question_form.createBy = this.userName;
      this.question_form.createId = this.loginName;
      this.question_open = true;
      this.question_title = "添加问题";
    },
    /** 删除按钮操作（需要区分两个角色删除的时机，提问者只有在问题状态为待处理时才可删除，其他时候不可删除） */
    handleDelete(row) {
      const recordId = row.recordId || this.ids || this.admin_ids;
      this.$confirm('是否确认删除选中数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delQa(recordId);
        }).then(response => {
          if(response.code === 200){
            this.getList();
    ...
    /** 修改问题按钮操作 （问题状态为待处理时才可修改，其他时候不可修改）*/
    handleUpdate(row) {
      this.resetQuestionForm();
      this.getTypeOptions();
      this.getSecurityOptions();
      ...
      getQa(recordId, this.mode).then(response => {
        this.question_form = {
          ...
          questionAttachList: response.data.questionAttachList || []
        };
        this.question_open = true;
        this.question_title = "修改问题";
      });
    },
    /** 采纳按钮操作（等问题状态变为已处理后才可进行采纳操作） */
    handleAccept(row) {
      const recordId = row.recordId || this.ids;
      this.$confirm('是否确认采纳选中数据项?',{
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return accQa(recordId);
        }).then(response => {
          if(response.code === 200){
            ...
            this.$message.error("采纳失败" || response.msg);
          }
        }).catch(error => {
    ...
    submitQuestionForm() {
      this.$refs["question_form"].validate(valid => {
        if (valid) {
          const isUpdate = !!this.question_form.recordId;
          const operation = isUpdate ? updateQa(this.question_form) : addQa(this.question_form);
          operation.then(response => {
            if(response.code === 200){
              this.$message.success(`${isUpdate ? '修改' : '新增'}成功`);
              this.question_open = false;
              this.getList();
              this.getSearchList(); // 刷新搜索表格
            }else{
    ...
    resetQuestionForm() {
      ...
      this.resetForm("question_form");
    },
    // 上传问题附件成功回调
    handleUploadQuestionSuccess(res, file, fileList) {
      if(res.code === 200){
        // 检查是否已存在相同ossId的附件
        const exists = this.question_form.questionAttachList.some(
        ...
        if(!exists) {
          ...
          this.question_form.questionAttachList.push(newAttachment);
          // 手动更新el-upload的文件列表
          this.$nextTick(() => {
            if(this.$refs.questionUpload) {
              this.$refs.questionUpload.uploadFiles = fileList.map(f => ({
                ...f,
    ...
    handleQuestionRemove(file, fileList) {
      // 从表单的questionAttachList数组中移除对应的附件
      let ossId = null;
      if(fileList.length === 0){
        this.question_form.questionAttachList.pop();
        ossId = file.ossId;
      }else{
        ossId = file.response.data.ossId;
        this.question_form.questionAttachList = this.question_form.questionAttachList.filter(
    ...
    handleUpdateAdmin(row) {
      ...
      this.getTypeOptions();
      this.getSecurityOptions();
      const recordId = row.recordId || this.admin_ids;
      this.mode = "admin";
      getQa(recordId, this.mode).then(response => {
        this.admin_form = {
          recordId: response.data.recordId,
        ...
        this.admin_open = true;
        this.admin_title = "管理员修改数据";
      });
    },
    /** 修改表单提交按钮 */
    submitAdminForm() {
      this.$refs["admin_form"].validate(valid => {
        if (valid) {
          const isUpdate = !!this.admin_form.recordId;
          if(isUpdate){
            updateQa(this.admin_form).then(response => {
              if(response.code === 200){
                this.$message.success('管理员修改成功');
                this.admin_open = false;
                this.getList();
                this.getSearchList(); // 刷新搜索表格
              }else
                this.$message.error("管理员修改失败" || response.msg);
            }).catch(error => {
              console.error('管理员修改失败', error);
            });
          }else{
            this.$message.error("无表单参数" || response.msg);
          }
        }
      });
    },
    ...
    resetAdminForm() {
      ...
      this.resetForm("admin_form");
    },
    // 管理员上传问题附件成功回调
    handleAdminUploadQuestionSuccess(res, file, fileList) {
      if(res.code === 200){
        // 检查是否已存在相同ossId的附件
        const exists = this.admin_form.questionAttachList.some(
          item => item.ossId === res.data.ossId
        );
        if(!exists) {
          const newAttachment = {
          ...
          this.admin_form.questionAttachList.push(newAttachment);
          // 手动更新el-upload的文件列表
          this.$nextTick(() => {
            if(this.$refs.adminQuestionUpload) {
              this.$refs.adminQuestionUpload.uploadFiles = fileList.map(f => ({
                ...f,
                status: f.status === 'success' ? 'success' : f.status,
                percentage: f.percentage || 100
              }));
    ...
    // 删除问题附件成功回调（删除sys_oss数据库中的内容）
    handleAdminQuestionRemove(file, fileList) {
      // 从表单的questionAttachList数组中移除对应的附件
      let ossId = null;
      if(fileList.length === 0){
        this.admin_form.questionAttachList.pop();
        ossId = file.ossId;
      }else{
        ossId = file.response.data.ossId;
        this.admin_form.questionAttachList = this.admin_form.questionAttachList.filter(
        item => item.ossId !== ossId
        );
      }
      if(!ossId){
    ...
    // 管理员上传回答附件成功回调
    handleAdminUploadAnswerSuccess(res, file, fileList) {
      if(res.code === 200){
        // 检查是否已存在相同ossId的附件
        const exists = this.admin_form.answerAttachList.some(
          item => item.ossId === res.data.ossId
        );
        if(!exists) {
          const newAttachment = {
          ...
          this.admin_form.answerAttachList.push(newAttachment);
          // 手动更新el-upload的文件列表
          this.$nextTick(() => {
            if(this.$refs.adminAnswerUpload) {
              this.$refs.adminAnswerUpload.uploadFiles = fileList.map(f => ({
                ...f,
                status: f.status === 'success' ? 'success' : f.status,
                percentage: f.percentage || 100
              }));
    ...
    // 删除问题附件成功回调（删除sys_oss数据库中的内容）
    handleAdminAnswerRemove(file, fileList) {
      // 从表单的questionAttachList数组中移除对应的附件
      let ossId = null;
      if(fileList.length === 0){
        this.admin_form.answerAttachList.pop();
        ossId = file.ossId;
      }else{
        ossId = file.response.data.ossId;
        this.admin_form.answerAttachList = this.admin_form.answerAttachList.filter(
        item => item.ossId !== ossId
        );
      }
      if(!ossId){
    ...
    submitExportForm() {
      this.$refs["export_form"].validate(valid => {
        if (valid) {
          expQa(this.export_form).then(response => {
            if(response.code === 200){
              this.$message.success('导出成功,文件路径为：' + response.msg);
