...
import com.sysware.system.domain.vo.MetaVo;
import com.sysware.system.domain.vo.RouterVo;
import com.sysware.system.mapper.SysMenuMapper;
import com.sysware.system.mapper.SysRoleMapper;
import com.sysware.system.mapper.SysRoleMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService {
    ...
    public List<RouterVo> buildMenus(List<SysMenu> menus) {
        ...
        for (SysMenu menu : menus) {
            ...
            router.setMeta(new MetaVo(menu.getMenuName(), menu.getIcon(), StrUtil.equals("1", menu.getIsCache()), menu.getRoutePath()));
            ...
            if (!cMenus.isEmpty() && UserConstants.TYPE_DIR.equals(menu.getMenuType())) {
                ...
                children.setPath(menu.getRoutePath());
                children.setComponent(menu.getComponent());
                children.setName(StrUtil.upperFirst(menu.getRoutePath()));
                children.setMeta(new MetaVo(menu.getMenuName(), menu.getIcon(), StrUtil.equals("1", menu.getIsCache()), menu.getRoutePath()));
				...
				List<RouterVo> childrenList = new ArrayList<RouterVo>();
				RouterVo children = new RouterVo();
				String routerPath = StringUtils.replaceEach(menu.getRoutePath(), new String[] { Constants.HTTP, Constants.HTTPS }, new String[] { "", "" });
				children.setPath(routerPath);
				children.setComponent(UserConstants.INNER_LINK);
				children.setName(StringUtils.capitalize(routerPath));
				children.setMeta(new MetaVo(menu.getMenuName(), menu.getIcon(), menu.getRoutePath()));
                childrenList.add(children);
                router.setChildren(childrenList);
            }
            routers.add(router);
    ...
    public boolean hasChildByMenuId(String menuId) {
        Long result = baseMapper.selectCount(new LambdaQueryWrapper<SysMenu>().eq(SysMenu::getParentId,menuId));
        return result > 0;
    }
    ...
    public boolean checkMenuNameUnique(SysMenu menu) {
        String menuId = Validator.isNull(menu.getMenuId()) ? "" : menu.getMenuId();
        SysMenu info = baseMapper.selectOne(new LambdaQueryWrapper<SysMenu>()
                .eq(SysMenu::getMenuName,menu.getMenuName())
                .eq(SysMenu::getParentId,menu.getParentId())
                .last("limit 1"));
        if (Validator.isNotNull(info) && !info.getMenuId().equals(menuId)) {
            return true;
        }
        return false;
    }
    ...
    @Override
    public List<SysMenu> selectPageList(SysMenu menu, String userId) {
        List<SysMenu> menuList = null;
        // 管理员显示所有菜单信息
        if (SysUser.isAdmin(userId)) {
            menuList = baseMapper.selectList(new LambdaQueryWrapper<SysMenu>()
    ...
    public String getRouteName(SysMenu menu) {
        String routerName = StrUtil.upperFirst(menu.getRoutePath());
        // 非外链并且是一级目录（类型为目录）
        if (isMenuFrame(menu)) {
    ...
    public String getComponent(SysMenu menu) {
		...
		} else if (StrUtil.isEmpty(menu.getComponent()) && menu.getParentId() != "" && isInnerLink(menu)) {
    ...
    public boolean isInnerLink(SysMenu menu) {
        return menu.getIsFrame().equals(UserConstants.NO_FRAME) && Validator.isUrl(menu.getRoutePath());
    }
