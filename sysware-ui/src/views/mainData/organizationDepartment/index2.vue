...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOrganizationDepartment(this.queryParams).then(response => {
        this.organizationDepartmentList = response.rows;
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有数据项?', "警告", {
        ...
        this.download('/mainData/organizationDepartment/export', {
          ...this.queryParams
        }, `组织部门数据表_${new Date().getTime()}.xlsx`)
      }).catch(() => {
