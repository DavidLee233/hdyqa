      ...
      <el-col style="flex: 0 0 36%; height: 100%; padding-right: 0px; overflow-y: auto;">
        <el-row :gutter="0" style="margin-bottom: 0px; height: 38vh; overflow: hidden;">
          ...
          <el-row :gutter="10" class="mb8" :style="{height: '2vh',  marginTop: '0px'}">
            ...
            <el-col :span="1.5">
              <el-button type="warning" plain icon="el-icon-view" size="mini" :disabled="multiple" @click="handleAccept">采纳</el-button>
            </el-col>
            <el-col :span="1.5">
        ...
        <el-row :gutter="10" class="mb8" :style="{height: '2vh',  marginTop: '0px'}">
          ...
          <el-col :span="1.5">
            <el-dropdown @command="handleReadCommand">
              <el-button type="success" plain size="mini">{{ filterSelf ? '仅查看自己' : '查看全部'}}<i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
...
<script>
import { listQuestionQa, listQuestionNotify, delQa, addQa, updateQa, accQa} from "@/api/qaSystem/qaQuestion";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listQuestionQa(this.queryParams).then(response => {
    ...
    /** 查询需提醒的问题回答列表 */
    getNotifyList(){
      listQuestionNotify().then(response => {
    ...
    handleNotify(list){
      ...
      // 1.拼接所有的note
      const content = `您有${list.length}条回答待采纳，请尽快处理！`;
      // 2.收集ids
    ...
    getSearchList() {
      ...
      this.searchQueryParams.pageType = "question";
    ...
    handleUpdate(row) {
      ...
      getQa(recordId, this.mode).then(response => {
        ...
        // TODO 将（修改的后端参数加入questionAttachList的参数）
        this.question_open = true;
