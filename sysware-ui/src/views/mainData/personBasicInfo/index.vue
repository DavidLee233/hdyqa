<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="personBasicInfoList" @selection-change="handleSelectionChange">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" label-position="right">
        ...
        <div class="form-section">
          ...
          <el-row :gutter="30">
            ...
            <el-col :span="8">
              <el-form-item label="专业" prop="postalCode">
                <el-input v-model="form.postalCode" placeholder="请输入专业名称"/>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
...
import {
  ...
  listRemotePersonBasicInfo,
  forceSyncRemotePersonBasicInfo
...
export default {
  ...
  methods: {
    ...
    async loadFieldMapping() {
      try {
        // 员工基本信息映射类型：2（兼容后端旧值1）
        const response = await getFieldMappingByType({ type: '2' });  // 查询员工基本信息字段映射列表
    ...
    convertToRemoteQuery(query) {
      ...
      if (query.mobile) remoteQuery.mobile = query.mobile;
      return remoteQuery;
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.pkPsndoc)
    ...
    handleUpdate(row) {
      ...
      this.reset();
      const pkPsnId = row.pkPsndoc || this.ids
      getPersonBasicInfo(pkPsnId).then(response => {
        // 已解决form传递值出现的问题
        this.form = response.data;
        this.open = true;
        this.title = "主数据员工基本信息修改数据";
      });
    },
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          const isUpdate = !!this.form.pkPsndoc;
          const operation = isUpdate ? updatePersonBasicInfo(this.form) : addPersonBasicInfo(this.form);
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除编号为"' + pkPsnId + '"的数据项？').then(function() {
        return delPersonBasicInfo(pkPsnId);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$confirm(confirmMessage, "警告", {
        ...
        let exportUrl = '/mainData/personBasicInfo/export';
        // 如果是远程数据，需要转换查询参数
        if (this.dataSource === 'remote') {
          queryParams = this.convertToRemoteQuery(this.queryParams);
          exportUrl = '/mainData/personBasicInfo/remote/export';
        }
        const filename = `员工基本信息数据_${this.dataSource === 'local' ? '本地' : '远程'}_${new Date().getTime()}.xlsx`;
    ...
    handleForceSync(syncMode) {
      ...
      this.$confirm(`是否确认${syncModeLabel}院主员工基本信息到本地？`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.syncLoading = true;
        forceSyncRemotePersonBasicInfo(mode).then(response => {
    ...
    /** 加载最近同步批次 */
    loadLatestSyncBatch() {
      listLatestMainDataSyncBatch('2', 1).then(response => {
        if (response.code === 200) {
